"""
Sign-up schemas module.

This module defines Pydantic models used for user registration.
It provides validation for user input, including username, email,
password, and repeat_password fields, with custom validators for
password complexity and password match.

Classes:
    SignUpModel (BaseModel): Pydantic model representing a sign-up form,
        validating username, email, password, and repeat_password fields
        with custom rules.

Example:
    ```python
    from src.schemas.auth.sign_up import SignUpModel
